...
AS $$
  SELECT EXISTS (
    ...
    WHERE id = _user_id
      AND LOWER(email) IN ('f.rahna@powerholding.com','j.chinokwetu@powerholding.com')
  );
$$;
...
BEGIN
  ...
  WITH target_users AS (
    ...
    WHERE ur.is_approved = true
      ...
      AND LOWER(p.email) NOT IN ('f.rahna@powerholding.com','j.chinokwetu@powerholding.com')
  ),
  video_stats AS (
    SELECT
      uva.user_id,
...
CREATE OR REPLACE FUNCTION public.get_pih_manager_summary()
RETURNS TABLE(
  total_users bigint,
  active_7d bigint,
...
BEGIN
  ...
  RETURN QUERY
  WITH team AS (
    SELECT * FROM public.get_pih_manager_team_progress()
  )
  SELECT
    COUNT(*)::bigint,
    COUNT(*) FILTER (WHERE is_active_7d)::bigint,
    COALESCE(SUM(initial_credits),0)::bigint,
    COALESCE(SUM(credit_balance),0)::bigint,
...
GRANT EXECUTE ON FUNCTION public.is_pih_manager(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_pih_manager_team_progress() TO authenticated;
GRANT EXECUTE ON FUNCTION public.get_pih_manager_summary() TO authenticated;

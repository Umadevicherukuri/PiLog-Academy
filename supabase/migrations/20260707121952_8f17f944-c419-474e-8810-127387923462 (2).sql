...
BEGIN
  ...
  ON CONFLICT (user_id, lesson_id) DO UPDATE SET
    ...
    best_score_percentage = CASE
      WHEN v_passed AND v_score_percentage > lesson_quiz_progress.best_score_percentage THEN v_score_percentage
      ELSE lesson_quiz_progress.best_score_percentage
    END,
    is_passed = lesson_quiz_progress.is_passed OR v_passed,
    passed_at = CASE
      WHEN v_passed AND lesson_quiz_progress.passed_at IS NULL THEN now()
